prettify.reproc <- function(filepath, WCpretty_dir = NULL,
  ...
  if (nrow(data) > 0) {
    colnames(data) <- c('PTT', 'Latitude1', 'Longitude1', 'Latitude2', 'Longitude2',
                        ...
                        'Satellite',	'Frequency',
                        'MsgCount', 'Error.radius',	'Semi.major.axis', 'Semi.minor.axis',
                        'Ellipse.orientation','GDOP..m.Hz.','Dist..Subsat.Track..deg.',
                        'X')
    ...
    if (!is.null(WCpretty_dir)) {
      prettyname <- paste0(WCpretty_dir, "/", filepath)
