...
simplify.archival <- function(tag, mindepth, mindur, divestart = 1, wetdry = 100, kclusters = NULL, wd.adjust = TRUE) {
  ...
  if ((pminbefore < (pfs*beh$start[1])) & (pbeforedeepdur >= mindur)) {
    ...
    max <- NA
    type <- 'Surface'
    shape <- Kmeans <- Borderline <- Deep <- Shallow <- NA
    row <- data.frame(start, end, dur, max, shape, type, Borderline, Kmeans, Deep, Shallow)
    beh <- rbind(row, beh)
  }
  ...
  if (('wet' %in% names(tag)) & (wd.adjust == TRUE)) {
    ...
    if (length(unique(wet)) == 2) {wetdry <- 0.5}
    wsamples <- c(1:length(wet)) / wfs
    drysamps <- wsamples[which(wet >= wetdry)]
    delete <- c()
    for (i in 1:length(drysamps)) {
      if (p[round(pfs*(drysamps[i]/wfs))] > 5) {
        delete <- c(delete, i)
      }
    }
    if (length(delete) > 0) {
      drysamps <- drysamps[-delete]
    }
    wetsamps <- wsamples[which(wet < wetdry)]
    delete <- c()
    for (i in 1:length(wetsamps)) {
      if (p[round(pfs*(wetsamps[i]/wfs))] < 5) {
        delete <- c(delete, i)
      }
    }
    if (length(delete) > 0) {
      wetsamps <- wetsamps[-delete]
    }
  ...
  # deep = below shallow threshold but not deep AND long enough to count as dive (e.g. not 50m and 30s)
  for (i in 1:nrow(beh)) {
    if (beh$type != 'Surface') {next}
    #Shallow
    pd <- p[(start*pfs):(end*pfs)]
    beh$Shallow[i] <- length(pd[which(pd < divestart)]) / pfs #anything above divestart is considered dry
