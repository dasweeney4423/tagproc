...
DF_land <- douglas.filter(Argos_data, argos_method = 'K', method = 'DAR', 
                          keep_lc = 2, maxredun = 3, duplrec = 'offset', 
                          minrate = 20, ratecoef = 25, r_only = FALSE)$all
```

```{r}
stime <- lubridate::ceiling_date(DF_land$Date[1], unit = 'days')
end <- lubridate::floor_date(DF_land$Date[nrow(DF_land)], unit = 'days')
crawl_times <- c()
time <- stime
while (time <= end) {
...
CRAWL_data <- crawl.apply(DF_land, model.interval = crawl_times, crs = 3310)
Ootrack <- CRAWL_data$preds
SL_data <- sun.moon(Ootrack)
BATHY_land <- bathy.sync(SL_data)
View(BATHY_land)
```
...
```{r}
write.csv(DF_land, paste0(WCpretty_dir,'/94815-DFLocs_2lc-3mr-20mr-25rc.csv'), row.names=F)

write.csv(BATHY_land, paste0(WCpretty_dir,'/94815-12hCRAWL_Environment.csv'), row.names=F)
