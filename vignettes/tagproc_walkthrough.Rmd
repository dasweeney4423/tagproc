...
Argos_data <- read.csv(paste0(WCpretty_dir,"/194277-Locations.csv"))
...
CRAWL_data <- crawl.apply(DF_data, model.interval = '2 hours', crs = 2230)

#add original data to crawl predictions
cpreds <- CRAWL_data$preds[,c("DeployID","Ptt","Date","LocationQuality","Latitude","Longitude","Type","Satellite","Error.radius","Error.Semi.major.axis","Error.Semi.minor.axis","Error.Ellipse.orientation")]
df_data <- DF_data[,c("DeployID","Ptt","Date","LocationQuality","Latitude","Longitude","Type","Satellite","Error.radius","Error.Semi.major.axis","Error.Semi.minor.axis","Error.Ellipse.orientation")]
locations <- rbind(cpreds,df_data)
...
tag.mapper(locations, zoom = 1, lon.offset = 0, lat.offset = 0,
...
POLY_data <- polyprox(locations, polypath = "C:/Users/marec/Docs/TagProcApp-20180519/20180519/Resources-Polygons")
...
```{r, fig.height=4, fig.width=7, fig.align='center'}
#pull in data
Beh_data <- read.csv(paste0(WCpretty_dir,"/194277-Behavior.csv"))

#split behavior data
#dive data will have kmeans clustering performed and borderline dives marked
split_Beh2 <- split.dive.msg(Beh_data, kclusters = 2)
split_Beh3 <- split.dive.msg(Beh_data, kclusters = 3)
split_Beh <- split_Beh2
...
DIVE_data <- bhvr.interpolate(split_Beh$Dives, locations)
...
```{r}
write.csv(locations, paste0(WCpretty_dir,'/DF_CRAWL_Locations.csv'), row.names = F)
